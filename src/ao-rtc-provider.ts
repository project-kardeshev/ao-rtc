...
import {
  EncryptedAoRTCContractConnectionState,
  User,
...
export interface AoRtc {
  ...
  getContractConnections(params: {
    userId: ArweaveID;
  }): Promise<Record<ArweaveID, EncryptedAoRTCContractConnectionState>>; // queries contract for all connections associated with a user
...
export class AoRtcProvider extends AoProvider implements AoRtc {
  ...
  async updateConnectionStates(): Promise<void> {
    ...
    for (const [connectionId, contractConnection] of Object.entries(
      ...
      if (host === publicKey) {
        ...
        await decryptJSONWithArconnect(
          contractConnection.Guest.IceCandidates,
          ...
          .then((candidates) =>
            candidates.forEach((candidate) =>
              this.connections[guest].addIceCandidate(candidate),
            ),
          );
        ...
        await decryptJSONWithArconnect(
          contractConnection.Host.IceCandidates,
          window.arweaveWallet,
        )
          .then((obj: object) =>
            Array.isArray(obj)
          ...
          .then((candidates) =>
            candidates.forEach((candidate) =>
              this.connections[host].addIceCandidate(candidate),
            ),
          );
  ...
  async connect({ id }: { id: ArweavePublicKey }): Promise<this> {
    ...
    const connectedAsHost = Object.entries(encryptedContractConnections).find(
      ([connectionId, value]: any) => {
        if (value.Host.id === address && value.Guest.id === id)
          return connectionId;
      },
    );
    // peer is attempting to connect as host to me
    const connectedAsGuest = Object.entries(encryptedContractConnections).find(
      ([connectionId, value]: any) => {
      ...
      if (event.candidate) {
        ...
        const messageId = await this.ao.message({
          ...
          tags: [
            ...
            {
              name: 'Candidate',
              value: await encryptJSONWithPublicKey(
                [event.candidate],
                remoteId,
              ),
            },
        ...
        console.debug(
          `ICE candidate sent to user ${remoteId} with message id ${messageId}`,
        );
      } else {
        // This null candidate event indicates the end of candidate gathering
        console.debug(
          'All ICE candidates have been gathered for peer ID:',
          remoteId,
        );
      }
    };
  ...
  > {
    ...
    const encryptedConnections = JSON.parse(Messages[0].Data) as Record<
      ArweavePublicKey,
      EncryptedAoRTCContractConnectionState
    >;
    ...
    return Object.fromEntries(
      Object.entries(encryptedConnections).filter(
        ([, value]: any) =>
          value.Host.id === userId || value.Guest.id === userId,
      ),
    );
  }
  ...
  async onRecieveOffer({ connectionId, hostPublicKey, encryptedOffer }) {
    try {
      const decryptedOffer = await decryptJSONWithArconnect(
        encryptedOffer,
        window.arweaveWallet,
      );
      ...
      await connection.setRemoteDescription(
        new RTCSessionDescription(decryptedOffer as RTCSessionDescriptionInit),
      );
  ...
  async onRecieveAnswer({ guestPublicKey, encryptedAnswer }) {
    try {
      const decryptedAnswer = await decryptJSONWithArconnect(
        encryptedAnswer,
        window.arweaveWallet,
      );
      const connection = this.connections[guestPublicKey];
      await connection.setRemoteDescription(
        new RTCSessionDescription(decryptedAnswer as RTCSessionDescriptionInit),
      );
    } catch (error) {
