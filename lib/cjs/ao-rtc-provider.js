...
exports.AoRtcProvider = void 0;
const aoconnect_1 = require("@permaweb/aoconnect");
const arweave_1 = __importDefault(require("arweave"));
const ao_provider_js_1 = require("./ao-provider.js");
...
class AoRtcProvider extends ao_provider_js_1.AoProvider {
    signer;
    ...
    async updateConnectionStates() {
        ...
        for (const [connectionId, contractConnection] of Object.entries(encryptedConnections)) {
            ...
            if (host === publicKey) {
                ...
                await (0, encryption_js_1.decryptJSONWithArconnect)(contractConnection.Guest.IceCandidates, window.arweaveWallet)
                ...
                const decryptedAnswer = contractConnection.ConnectionConfig.Answer
                    .length
                    ? new RTCSessionDescription((await (0, encryption_js_1.decryptJSONWithArconnect)(contractConnection.ConnectionConfig.Answer, window.arweaveWallet)))
                    : undefined;
                ...
                await (0, encryption_js_1.decryptJSONWithArconnect)(contractConnection.Host.IceCandidates, window.arweaveWallet)
                    .then((obj) => Array.isArray(obj)
                    ? obj.map((candidate) => new RTCIceCandidate(candidate))
                    : [])
                ...
                const decryptedOffer = contractConnection.ConnectionConfig.Offer.length
                    ? new RTCSessionDescription((await (0, encryption_js_1.decryptJSONWithArconnect)(contractConnection.ConnectionConfig.Offer, window.arweaveWallet)))
                    : undefined;
    ...
    async register(params) {
        const registrationId = await this.ao.message({
            ...
            signer: (0, aoconnect_1.createDataItemSigner)(window.arweaveWallet),
        });
    ...
    async connect({ id }) {
        ...
        const connectionId = await this.ao.message({
            ...
            tags: [
                ...
                { name: 'Guest', value: id },
                { name: 'Offer', value: await (0, encryption_js_1.encryptJSONWithPublicKey)(offer, id) },
            ],
            signer: (0, aoconnect_1.createDataItemSigner)(window.arweaveWallet),
        });
            ...
            if (event.candidate) {
                ...
                const messageId = await this.ao.message({
                    ...
                    tags: [
                        ...
                        {
                            name: 'Candidate',
                            value: await (0, encryption_js_1.encryptJSONWithPublicKey)([event.candidate], remoteId),
                        },
            ...
            try {
                ...
                this.ao.message({
                    ...
                    tags: [
                        ...
                        {
                            name: 'Offer',
                            value: await (0, encryption_js_1.encryptJSONWithPublicKey)(offer, remoteId),
                        },
                        { name: 'ConnectionId', value: connectionId },
                    ],
                    signer: (0, aoconnect_1.createDataItemSigner)(window.arweaveWallet),
    ...
    async onRecieveOffer({ connectionId, hostPublicKey, encryptedOffer }) {
        try {
            const decryptedOffer = await (0, encryption_js_1.decryptJSONWithArconnect)(encryptedOffer, window.arweaveWallet);
            ...
            this.ao.message({
                ...
                tags: [
                    ...
                    {
                        name: 'Answer',
                        value: await (0, encryption_js_1.encryptJSONWithPublicKey)(answer, hostPublicKey),
                    },
                    { name: 'ConnectionId', value: connectionId },
                ],
                signer: (0, aoconnect_1.createDataItemSigner)(window.arweaveWallet),
    ...
    async onRecieveAnswer({ guestPublicKey, encryptedAnswer }) {
        try {
            const decryptedAnswer = await (0, encryption_js_1.decryptJSONWithArconnect)(encryptedAnswer, window.arweaveWallet);
...
exports.AoRtcProvider = AoRtcProvider;
